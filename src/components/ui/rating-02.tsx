...
const RATINGS = [
  ...
  {
    folder: "Disappointed face",
    file: "disappointed_face_3d.png",
    label: "Bad",
  },
  { folder: "Neutral face", file: "neutral_face_3d.png", label: "Okay" },
  { folder: "Smiling face", file: "smiling_face_3d.png", label: "Good" },
  {
    folder: "Grinning face with smiling eyes",
    file: "grinning_face_with_smiling_eyes_3d.png",
    label: "Awesome",
  },
] as const;
  ...
  return (
    ...
    >
      {RATINGS.slice(0, max).map((rating, index) => {
        ...
        const isFocusTarget =
          Math.max(Math.ceil(currentValue), 1) - 1 === index;
        ...
        return (
          <motion.button
            ...
            disabled={disabled}
            className="group flex flex-col items-center gap-1.5 rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-[#FF3D77] focus-visible:ring-offset-2"
            initial={
              prefersReducedMotion ? false : { opacity: 0, y: 12, scale: 0.8 }
            }
