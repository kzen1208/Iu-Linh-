...
export type HaloReelItem = {
  ...
  src?: string;
  alt?: string;
  /** Tên file khi bấm tải ảnh này về máy (chỉ áp dụng khi có src) */
  downloadName?: string;
...
export interface HaloReelProps
  ...
  /** Hiện nút tải ảnh về máy trên mỗi thẻ có ảnh thật. @default true */
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "focus-visible:ring-2 focus-visible:ring-[#FF3D77] focus-visible:ring-inset",
        className,
      )}
      style={style}
      {...props}
...
function WheelCard({
  ...
  // thẻ tự co giãn theo vị trí trên vòng — nút tải cần hủy scale đó để luôn
  // giữ kích thước cố định, không bị tí hin lại ở các thẻ phía sau
  ...
  return (
    ...
    >
      <div className="relative h-full w-full overflow-hidden rounded-2xl">
        {item.src ? (
          ...
          <div
            ...
            style={{
              backgroundColor: item.bgColor ?? "#FFF3E9",
              color: item.textColor ?? "#2B2230",
            }}
          >
            {item.title ? (
      ...
      {!decorative && item.src && showDownload && (
        <motion.div
        ...
        >
          <AnimatedDownloadButton
            href={item.src}
            fileName={item.downloadName ?? `khoanh-khac-${index + 1}.jpg`}
            label="Tải"
            size={22}
            expandedWidth={78}
          />
